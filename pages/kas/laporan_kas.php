...
$mode = isset($_GET['mode']) ? $_GET['mode'] : 'all'; // Mode: 'penjualan' (grafik) atau 'all'

// --- FILTER KATEGORI PENTING ---
// Jika Mode Grafik Penjualan -> Tampilkan Penjualan Harian + QRIS
// Jika Mode Laporan Kas (All) -> Tampilkan SEMUA KECUALI Seragam & Eskul (Karena masuk Laporan Distribusi)
$filter_sql = "";
$judul_halaman = "Laporan Arus Kas Koperasi";

if($mode == 'penjualan'){
    $judul_halaman = "Grafik & Rekap Penjualan Koperasi";
    $filter_sql = "AND kategori IN ('penjualan_harian', 'qris_masuk')";
} else {
    // EXCLUDE DISTRIBUSI (SERAGAM/ESKUL)
    $filter_sql = "AND kategori NOT IN ('penjualan_seragam', 'penjualan_eskul')";
}
...
// Cek apakah bulan lalu sudah ditutup?
...
if(isset($_GET['export_excel'])){
    ...
    header("Content-Disposition: attachment; filename=Laporan_Kas_$tgl_awal.xls");
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <?php if($mode == 'all'): ?>
            <small class="text-muted">Hanya menampilkan omzet Stok Koperasi (ATK/Jajanan) & Operasional.</small>
        <?php endif; ?>
    </div>
</div>
...
<div class="card border-0 shadow-sm mb-4 bg-gradient-warning text-white">
    <div class="card-body p-4 d-flex justify-content-between align-items-center">
        <div>
            ...
            <p class="mb-0 opacity-75">Laporan periode <strong><?= $nama_bulan_lalu ?></strong> belum dikunci. Segera tutup buku untuk mengamankan data.</p>
        </div>
        <form action="process/tutup_buku_aksi.php" method="POST">
            ...
            <button type="submit" name="auto_tutup_buku" class="btn btn-light text-warning fw-bold shadow-sm px-4" onclick="return confirm('Yakin ingin menutup buku bulan <?= $nama_bulan_lalu ?>? Data tidak bisa diubah lagi.')">
                <i class="fas fa-lock me-2"></i> Tutup Buku Sekarang
            </button>
        </form>
    </div>
</div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    if(empty($transaksi)): ?>
                        <tr><td colspan="<?= $mode=='all'?6:4 ?>" class="text-center py-5 text-muted">Tidak ada transaksi pada periode ini.</td></tr>
                    <?php endif;
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if($row['kategori'] == 'penjualan_harian'): ?>
                                <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25">PENJUALAN KOPERASI</span>
                ...
                <tfoot class="bg-light fw-bold border-top">
                    <tr>
                        <td colspan="3" class="text-center text-uppercase small ls-1 text-muted py-3">Total Periode Ini</td>
